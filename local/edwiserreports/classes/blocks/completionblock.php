...
class completionblock extends utility {
    ...
    public static function get_completions($courseid, $cohortid) {
        ...
        foreach ($enrolledstudents as $user) {
            ...
            $completioninfo->enrolledon = $user->firstname; //quyen date("d M Y", $enrolinfo->timemodified);
            $completioninfo->enrolltype = $user->lastname; //$enrolinfo->enrol;
			$completioninfo->lastaccess = $user->email;//$lastvisits;
			$completioninfo->compleiontime = $user->phone2;//self::get_timecompleted($courseid, $user->id);
